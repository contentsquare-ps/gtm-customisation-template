___TERMS_OF_SERVICE___

By creating or modifying this file you agree to Google Tag Manager's Community
Template Gallery Developer Terms of Service available at
https://developers.google.com/tag-manager/gallery-tos (or such other URL as
Google may provide), as modified from time to time.


___INFO___

{
  "type": "TAG",
  "id": "cvt_temp_public_id",
  "version": 1,
  "securityGroups": [],
  "displayName": "Contentsquare - tracking customisation",
  "brand": {
    "id": "brand_dummy",
    "displayName": "",
    "thumbnail": "data:image/png;base64,\u003d\u003d"
  },
  "description": "Deploy Contentsquare\u0027s pageviews, dynamic variables and page events from a single template. \nCustomise your implementation with a single click.",
  "containerContexts": [
    "WEB"
  ]
}


___TEMPLATE_PARAMETERS___

[
  {
    "type": "SELECT",
    "name": "action_type",
    "displayName": "Action type",
    "macrosInSelect": false,
    "selectItems": [
      {
        "value": "artificial_pageview",
        "displayValue": "Artificial pageview"
      },
      {
        "value": "standard_page_event",
        "displayValue": "Standard page event"
      },
      {
        "value": "dynamic_variable",
        "displayValue": "Dynamic variable"
      },
      {
        "value": "ets_page_event",
        "displayValue": "ETS page event"
      },
      {
        "value": "etp_page_event",
        "displayValue": "ETP page event"
      },
      {
        "value": "add_to_cart",
        "displayValue": "Add to cart"
      }
    ],
    "simpleValueType": true,
    "notSetText": "Select an action"
  },
  {
    "type": "GROUP",
    "name": "Artificial pageview group",
    "displayName": "",
    "groupStyle": "NO_ZIPPY",
    "subParams": [
      {
        "type": "LABEL",
        "name": "artificial_pageview_label",
        "displayName": "With this action you can fire a new artificial pageview with a customised query.\n\nInsert below the query parameter to be used or leave it empty to collect the default page URL"
      },
      {
        "type": "TEXT",
        "name": "artificial_pageview_query",
        "displayName": "Artificial pageview query",
        "simpleValueType": true
      }
    ],
    "enablingConditions": [
      {
        "paramName": "action_type",
        "paramValue": "artificial_pageview",
        "type": "EQUALS"
      }
    ]
  },
  {
    "type": "GROUP",
    "name": "Dynamic variables group",
    "displayName": "",
    "groupStyle": "NO_ZIPPY",
    "subParams": [
      {
        "type": "LABEL",
        "name": "dynamic_variables_label",
        "displayName": "With this action you can fire new dynamic variables with custom keys and values.\n\nYou can choose to send 1 dynamic variable for each tag or nest them all together by appending multiple rows with the button below.\nEnsure to select the correct value type."
      },
      {
        "type": "SIMPLE_TABLE",
        "name": "dynamic_variables_table",
        "displayName": "Insert below the dynamic variables you\u0027d like to collect with this tag",
        "simpleTableColumns": [
          {
            "defaultValue": "",
            "displayName": "Dynamic variable key",
            "name": "dynamic_variable_key",
            "type": "TEXT",
            "valueValidators": [
              {
                "type": "NON_EMPTY"
              }
            ]
          },
          {
            "defaultValue": "",
            "displayName": "Dynamic variable value",
            "name": "dynamic_variable_value",
            "type": "TEXT",
            "valueValidators": [
              {
                "type": "NON_EMPTY"
              }
            ]
          },
          {
            "defaultValue": "",
            "displayName": "Value type",
            "name": "value_type",
            "type": "SELECT",
            "selectItems": [
              {
                "value": "value_string",
                "displayValue": "String"
              },
              {
                "value": "value_integer",
                "displayValue": "Integer"
              }
            ],
            "valueValidators": [
              {
                "type": "NON_EMPTY"
              }
            ]
          }
        ],
        "newRowButtonText": "Add new dynamic variable",
        "notSetText": "",
        "valueValidators": [
          {
            "type": "NON_EMPTY"
          }
        ]
      }
    ],
    "enablingConditions": [
      {
        "paramName": "action_type",
        "paramValue": "dynamic_variable",
        "type": "EQUALS"
      }
    ]
  },
  {
    "type": "GROUP",
    "name": "Page event group",
    "displayName": "",
    "groupStyle": "NO_ZIPPY",
    "subParams": [
      {
        "type": "LABEL",
        "name": "page_event_label",
        "displayName": "With this action you can fire new page event with a custom value.\n\nYou can either hard-code it by typing it in the below field or choose a variable from your datalayer."
      },
      {
        "type": "TEXT",
        "name": "page_event_value",
        "displayName": "Page event value",
        "simpleValueType": true,
        "valueValidators": [
          {
            "type": "NON_EMPTY"
          }
        ]
      }
    ],
    "enablingConditions": [
      {
        "paramName": "action_type",
        "paramValue": "standard_page_event",
        "type": "EQUALS"
      }
    ]
  },
  {
    "type": "GROUP",
    "name": "ETP group",
    "displayName": "",
    "groupStyle": "NO_ZIPPY",
    "subParams": [
      {
        "type": "LABEL",
        "name": "etp_label",
        "displayName": "With this action you can fire new ETP event with a custom value.\n\nYou can either hard-code it by typing it in the below field or choose a variable from your datalayer"
      },
      {
        "type": "TEXT",
        "name": "etp_value",
        "displayName": "ETP event value",
        "simpleValueType": true,
        "valueValidators": [
          {
            "type": "NON_EMPTY"
          }
        ]
      }
    ],
    "enablingConditions": [
      {
        "paramName": "action_type",
        "paramValue": "etp_page_event",
        "type": "EQUALS"
      }
    ]
  },
  {
    "type": "GROUP",
    "name": "ETS group",
    "displayName": "",
    "groupStyle": "NO_ZIPPY",
    "subParams": [
      {
        "type": "LABEL",
        "name": "ets_label",
        "displayName": "With this action you can fire new ETS event with a custom value\n\nYou can either hard-code it by typing it in the below field or choose a variable from your datalayer"
      },
      {
        "type": "TEXT",
        "name": "ets_value",
        "displayName": "ETS event value",
        "simpleValueType": true,
        "valueValidators": [
          {
            "type": "NON_EMPTY"
          }
        ]
      }
    ],
    "enablingConditions": [
      {
        "paramName": "action_type",
        "paramValue": "ets_page_event",
        "type": "EQUALS"
      }
    ]
  },
  {
    "type": "GROUP",
    "name": "Add to cart group",
    "displayName": "",
    "groupStyle": "NO_ZIPPY",
    "subParams": [
      {
        "type": "LABEL",
        "name": "add_to_cart_label",
        "displayName": "With this action you can fire a new add to cart event by providing the SKU of the item added to the bag.\n\nPlease note that this event is only usable if you are a CS Merchandising client."
      },
      {
        "type": "TEXT",
        "name": "add_to_cart_sku",
        "displayName": "SKU",
        "simpleValueType": true,
        "valueValidators": [
          {
            "type": "NON_EMPTY"
          }
        ]
      }
    ],
    "enablingConditions": [
      {
        "paramName": "action_type",
        "paramValue": "add_to_cart",
        "type": "EQUALS"
      }
    ]
  }
]


___SANDBOXED_JS_FOR_WEB_TEMPLATE___

const copyFromWindow = require ('copyFromWindow');
const getUrl = require('getUrl');
const getQueryParameters = require('getQueryParameters');
const encodeUriComponent = require('encodeUriComponent');
const createQueue = require ('createQueue');
const makeInteger = require('makeInteger');
const makeNumber = require('makeNumber');
const getType = require('getType');

const logToConsole = require('logToConsole');

let _uxaPush = createQueue('_uxa');
let windowPath = getUrl('path');
let windowHash = getUrl('fragment');

switch(data.action_type){
  case 'artificial_pageview':
    if (windowHash !== '') {
      if(typeof data.artificial_pageview_query == 'undefined'){
        _uxaPush(['trackPageview', windowPath+'?__'+windowHash]);
      }
      else{
        _uxaPush(['trackPageview', windowPath]);
      }
    }
    else {
      if(typeof data.artificial_pageview_query == 'undefined'){
        _uxaPush(['trackPageview', windowPath]);
      }
      else{
        _uxaPush(['trackPageview', windowPath+'?cs_query='+data.artificial_pageview_query]);
      }
    }
    data.gtmOnSuccess();
    break;
  case 'dynamic_variable':
    var checkedValue;
    var dvarValue;
    for (var i = 0; i < data.dynamic_variables_table.length; i++) {
      //logToConsole('value type ' + data.dynamic_variables_table[i].value_type);
      dvarValue = data.dynamic_variables_table[i].dynamic_variable_value;
      if(data.dynamic_variables_table[i].value_type =='value_integer'){
        dvarValue = makeInteger(data.dynamic_variables_table[i].dynamic_variable_value);
      }
      _uxaPush(
        ["trackDynamicVariable", {
          key: data.dynamic_variables_table[i].dynamic_variable_key, 
          value: dvarValue
        }]
      );
      logToConsole('sent result ' + dvarValue + ' ' + (typeof dvarValue) );
    }
    data.gtmOnSuccess();
    break;
  case 'standard_page_event':
     _uxaPush(['trackPageEvent', data.page_event_value]);
    data.gtmOnSuccess();
    break;
  case 'ETP_page_event':
     _uxaPush(['trackEventTriggerRecording', '@ETP@'+data.etp_value]);
    data.gtmOnSuccess();
    break;
  case 'ETS_page_event':
     _uxaPush(['trackEventTriggerRecording', '@ETS@'+data.ets_value]);
     data.gtmOnSuccess();
    break;
  case 'add_to_cart':
     _uxaPush(['ec:cart:add', {sku: data.add_to_cart_sku}]);
    data.gtmOnSuccess();
    break;
}


___WEB_PERMISSIONS___

[
  {
    "instance": {
      "key": {
        "publicId": "access_globals",
        "versionId": "1"
      },
      "param": [
        {
          "key": "keys",
          "value": {
            "type": 2,
            "listItem": [
              {
                "type": 3,
                "mapKey": [
                  {
                    "type": 1,
                    "string": "key"
                  },
                  {
                    "type": 1,
                    "string": "read"
                  },
                  {
                    "type": 1,
                    "string": "write"
                  },
                  {
                    "type": 1,
                    "string": "execute"
                  }
                ],
                "mapValue": [
                  {
                    "type": 1,
                    "string": "_uxa"
                  },
                  {
                    "type": 8,
                    "boolean": true
                  },
                  {
                    "type": 8,
                    "boolean": true
                  },
                  {
                    "type": 8,
                    "boolean": false
                  }
                ]
              }
            ]
          }
        }
      ]
    },
    "clientAnnotations": {
      "isEditedByUser": true
    },
    "isRequired": true
  },
  {
    "instance": {
      "key": {
        "publicId": "get_url",
        "versionId": "1"
      },
      "param": [
        {
          "key": "urlParts",
          "value": {
            "type": 1,
            "string": "any"
          }
        },
        {
          "key": "queriesAllowed",
          "value": {
            "type": 1,
            "string": "any"
          }
        }
      ]
    },
    "clientAnnotations": {
      "isEditedByUser": true
    },
    "isRequired": true
  },
  {
    "instance": {
      "key": {
        "publicId": "logging",
        "versionId": "1"
      },
      "param": [
        {
          "key": "environments",
          "value": {
            "type": 1,
            "string": "debug"
          }
        }
      ]
    },
    "clientAnnotations": {
      "isEditedByUser": true
    },
    "isRequired": true
  }
]


___TESTS___

scenarios:
- name: dvar test
  code: |-
    const mockData = {
      action_type: 'dynamic_variable',
      dynamic_variables_table:[
        {
          dynamic_variable_key:'test 1',
          dynamic_variable_value:'test test',
          value_type:'value_string'
        },
        {
          dynamic_variable_key:'test 2',
          dynamic_variable_value:'5.89',
          value_type:'value_integer'
        }
        ]
    };

    // Call runCode to run the template's code.
    runCode(mockData);

    // Verify that the tag finished successfully.
    assertApi('gtmOnSuccess').wasCalled();


___NOTES___

Created on 21/09/2021, 11:44:50


